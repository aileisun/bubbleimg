...
Usage:   alignstamp(obj, bands=('u','g','r','i','z'), band_rf='r', xwidth=57, ywidth=57)
...
def objw_makeall(obj, bands=('u','g','r','i','z'), band_rf='r', xwidth=57, ywidth=57, tods9=False):
	"""
	PURPOSE: make aligned stamp multi-band images of an SDSS object. 
	...
	INPUT: 
		...
		xwidth=57 (int):	 width of the image
		ywidth=57 (int):	 height of the image
	...
	dir_obj=obj.dir_obj
	if not os.path.isdir(dir_obj): os.makedirs(dir_obj)
	...
	# saving sdss photoobj (which is not directly used but for easy viewing)
	filename=dir_obj+'PhotoObj.csv'
	if not os.path.isfile(filename): obj.sdss.photoobj.write(filename,format='ascii',delimiter=',')
...
def cutstampImage(image, xcenter, ycenter, xwidth, ywidth):
	"""
	PURPOSE: Cut out a stamp image from image
...
def getalignedstampImages(obj, bands=('g','r','i'), band_rf='r', xwidth=57, ywidth=57, savefits=True,clipnegative=False):
	"""

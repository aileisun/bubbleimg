...
# 	obj=obsobj(listmagellan[1]) # 2100
	
# 	# obj.dir_obj=dir_data+'M'+str(obj.galaxy.OBJID)+'/'

# 	image=subtractimg.subtractStampImg(obj,a1=1.,a2=0.5)


# 	d.set_np2arr(img)

def objw_makeall(obj, bandline='r',bandconti='z'):
	"""
...
def objw_stamp_lOIII5008_F(obj, bandline='r',bandconti='z'):
	...
	def intRoverldl(band='r'):
		...
		"""
		R,l=fromspec.getFilterResponseFunc(band=band)
	...
	if z < z1 or z > z2: print ("WARNING: Invalid redshift. OIII falls outside filter 60 percent transmission range. ")
		...
		intR=intRoverldl(band=bandline)
		...
		RHb,ROIII4959,ROIII5007=R_lambda(lHb*(1.+z),band=bandline),R_lambda(lOIII4959*(1.+z),band=bandline),R_lambda(lOIII5008*(1.+z),band=bandline)
...
def objw_stamp_lOIII5008_I(obj):
	"""
	PURPOSE: infer OIIIl5007 intensity map [erg cm-2 s-1 arcsec-2] from flux maps stamp_F_lOIII5008. 

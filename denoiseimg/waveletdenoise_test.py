...
"""
Do wavelet denoising on images. 
...
def main(directory='./'):
    ...
    # if not os.path.isdir(directory+wname):
    #     os.mkdir(directory+wname)

    # # run two thresholds 0.5*sigma and 1*sigma
    # # run_batch(img,thres_factor=0.5,wavelet=wavelet,directory=directory+wname+'/threshold_05sigma/',suffix='')
    # run_batch(img,thres_factor=1.,wavelet=wavelet,directory=directory+wname+'/threshold_1sigma/',suffix='')
...
def run_batch(img, thres_factor, wavelet,directory,suffix):
    ...
    # write summary
    tsummary=Table([[sigma],[thres],['factor '+str(thres_factor)],[wavelet.name]],names=['sigma','thres_denoise','thres_denoise_type','wavelet'])

...
class sumireimgLoader(imgLoader):
	...
	def __init__(self, **kwargs):
		...
		Load hsc images within the iaa sumire cluster using lsst stack. Load calexp cutouts instead of plain stamp. 
	...
	# 			print "[sumireimgloader] running _download_psfs_at_iaa()"
	# 			status = self._download_psfs_at_iaa()
	# 			return status
	# 		else: 
	# 			print "[sumireimgloader] skip _download_psfs_at_iaa() as file exists"
	# 			return True


	# def _download_psf_at_iaa(self, band):
	# 	if self.environment!='iaa':
	# 		raise Exception("[sumireimgLoader] _download_psf_at_iaa() can only be called when environment is iaa")
